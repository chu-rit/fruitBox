...
$cssReset = "      html, body { height: 100dvh; margin: 0; padding: 0; background-color: #FFF8E7; }`n      body { overflow: auto; overscroll-behavior: none; }`n      #root { display: flex; height: 100dvh; flex: 1; }"
...
if ($html -notmatch 'rel="manifest"') {
  $heightFixScript = '<script>(function(){var minH=Infinity;var count=0;var maxCount=20;var settled=false;function measure(){var h=window.visualViewport?window.visualViewport.height:window.innerHeight;if(h<minH)minH=h;count++;if(count<maxCount&&!settled){requestAnimationFrame(measure);}else{applyHeight(minH);settled=true;}}function applyHeight(h){document.documentElement.style.setProperty("--vh",h*0.01+"px");document.documentElement.style.height=h+"px";document.body.style.height=h+"px";var root=document.getElementById("root");if(root)root.style.height=h+"px";}window.addEventListener("load",function(){window.scrollTo(0,1);setTimeout(measure,50);});})();</script>'
