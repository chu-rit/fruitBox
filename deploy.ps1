...
$cssReset = "      :root { --vh: 1vh; }`n      html, body { height: calc(var(--vh) * 100); margin: 0; padding: 0; background-color: #FFF8E7; }`n      body { overflow: hidden; }`n      #root { display: flex; height: calc(var(--vh) * 100); flex: 1; }"
...
if ($html -notmatch 'rel="manifest"') {
  $heightFixScript = '<script>(function(){function fixHeight(){var h=window.visualViewport?window.visualViewport.height:window.innerHeight;document.documentElement.style.setProperty("--vh",h*0.01+"px");document.documentElement.style.height=h+"px";document.body.style.height=h+"px";var root=document.getElementById("root");if(root)root.style.height=h+"px";}window.addEventListener("load",function(){fixHeight();setTimeout(fixHeight,100);setTimeout(fixHeight,500);setTimeout(fixHeight,1000);});window.addEventListener("resize",fixHeight);if(window.visualViewport)window.visualViewport.addEventListener("resize",fixHeight);})();</script>'
