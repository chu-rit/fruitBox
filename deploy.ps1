...
$cssReset = "      html, body { height: 100%; margin: 0; padding: 0; background-color: #FFF8E7; }`n      body { overflow: hidden; overscroll-behavior: none; }`n      #root { display: flex; height: 100%; flex: 1; }"
...
if ($html -notmatch 'rel="manifest"') {
  $heightFixScript = '<script>(function(){function applyHeight(h){document.documentElement.style.setProperty("--vh",h*0.01+"px");document.documentElement.style.height=h+"px";document.body.style.height=h+"px";var root=document.getElementById("root");if(root)root.style.height=h+"px";}function getH(){return window.visualViewport?window.visualViewport.height:window.innerHeight;}window.addEventListener("load",function(){window.scrollTo(0,1);setTimeout(function(){applyHeight(getH()+50);},100);if(window.visualViewport){window.visualViewport.addEventListener("resize",function(){applyHeight(getH());});}});})();</script>'
