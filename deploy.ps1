...
if ($html -notmatch 'rel="manifest"') {
  $scrollTriggerScript = '<script>(function(){window.addEventListener(''load'',function(){console.log(''Page loaded'');setTimeout(function(){console.log(''Dispatching scroll event'');window.dispatchEvent(new Event(''scroll''));console.log(''Scroll event dispatched'');},300);});})();</script>'
