...
if ($html -notmatch 'rel="manifest"') {
  $vhScript = '<script>(function(){var ticking=false;function setVH(){document.documentElement.style.setProperty(''--vh'',(window.innerHeight*0.01)+''px'');ticking=false;}window.addEventListener(''scroll'',function(){if(!ticking){requestAnimationFrame(setVH);ticking=true;}});setTimeout(function(){requestAnimationFrame(setVH);},100);window.addEventListener(''resize'',setVH);})();</script>'
